...
verifycodepath = 'http://123.59.79.146:8089/servlet/NewImage'
path = 'http://123.59.79.146:8089/servlet/json'
trade=True
capppath = 'http://t.c.jzsec.com'
...
def get_publickey(session=None):
    ...
    response = session.get(path, params=params).json()
...
def get_captcha(session=None):
    ...
    response = session.get(verifycodepath, params=params)
    return r, response
    

def recognize_captcha(r=None, data=None):
    if data is None:
        return '0000'
...
def login(session=None):
    publickey = get_publickey(session=session)
    error_no = '1'

    while error_no != '0':
        r, response = get_captcha(session)
        if 'JSESSIONID' in response.cookies.keys():
            template['sessionid'] = response.cookies['JSESSIONID']
        ticket = recognize_captcha(r, response.content)
        params = {
            'branch_no': '',
            'input_content': 81106500003,
            'ticket': ticket,
            'entrust_way': 9,
        ...
        ret = session.post(path, data=params, headers=headers).json()
...
#     print(ret)
    ...
    funcNo301503 = {
            ...
            'sessionid': sessionid ,
            'fund_account': 81106500003,
            'entrust_way': 'G',
            'branch_no': '0811',
            'cust_code': 81106500003,
            'password': 'encrypt_rsa%',
            #'password':'123123',
            'funcNo': 301503,
    ...
    bool,temp=test.interfaceTest(path + ";jsessionid=" + sessionid,funcNo301515,trade=trade)
